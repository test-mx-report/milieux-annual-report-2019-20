...
vis = d3.select('#project-highlights').append('svg').attr('width', x).attr('height', y)
...
d3.json('/_annual-reports/2019/assets/data/project-highlights.json', function (json) {
  root = json
  root.fixed = true
  root.x = x / 2
  root.y = y / 2
...
function update () {
  ...
  var images = nodeEnter.append('a')
    ...
    .attr('data-type', 'project-highlights')
  ...
  nodeEnter.append('text')
    .text(function (d) { if (d.project) return d.project })
    ...
    .on('mouseenter', function (d, i, j, k) {
      ...
      if (d.event) {
        d3.select(this)
          ...
          .attr('height', 100)
          .attr('width', 100)
        d3.select('#project-highlights-sidebar').classed('show', true)
        d3.select('#project-highlights-name').text(d.event)
        d3.select('#project-highlights-member').text(d.parent)
        d3.select('#project-highlights-info').classed('hide', true)
      }
    })
  // set back
    .on('mouseleave', function (d) {
      d3.select(this)
        ...
        .attr('height', 50)
        .attr('width', 50)
      d3.select('#project-highlights-sidebar').classed('show', false)
      d3.select('#project-highlights-info').classed('hide', false)
      // if (d.project) d3.select('#project-highlights-member').text('')
      // if (d.parent) d3.select('#project-highlights-member').text('')
      // if (d.event) d3.select('#project-highlights-name').text('')
    })
